...
class Model():
    ...
    def optimize(self):
        ...
        qvals,reward = self.model(s1)
        qvals = qvals.gather(1,a)
        rewards = reward.gather(1,a)

        with torch.no_grad():
            qvals_t,reward_t = self.target_net(s2)
            qvals_t = qvals_t + reward_t
            qvals_t = qvals_t.max(1)[0].unsqueeze(1)

        expected_qval = qvals_t * self.GAMMA * (1-d) + r1

        loss= self.l2(qvals + reward,expected_qval)
        loss = loss.mean()
    ...
    def select_caction(self,state):
        ...
        with torch.no_grad():
            ...
            a,_ = self.model((v,frame))
            probs = F.softmax(10 * a)       #10 = beta for softmax calculation with beta emphasis on larger values
            m = Categorical(probs)
            action = m.sample()
            idx = action.item()
            a1,a2,a3,a4,a5 = np.where(np.arange(243).reshape((3,3,3,3,3)) == idx)
    ...
    #STOCHASTIC ACTION SELECTION WITH DECAY TOWARDS GREEDY SELECTION. Actions are selected based on max of Q value
